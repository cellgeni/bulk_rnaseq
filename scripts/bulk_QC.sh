...
printf "Sample\tPaired\tStrand\tSalmon_lt\tRead_len\tMapped_len\tN_reads\tN_uniq\tN_multi\t"
printf "Pct_uniq\tPct_M1\tPct_M2\tPct_U1\tPct_U2\tPct_U3\t"
printf "STAR_U\tFcounts\tRSEM\tSalmon_aln\tSalmon_reads\n"
...
do
    ...
    then
        ...
        LT=`perl -ne 'print "$1" if (m/libType.*{ (.*?) }/)' $i/salmon_reads.log`
        if [[ $LT == "A" ]]
        then
            LT=`perl -ne 'print "$1" if (m/most likely library type as (.*?)$/)' $i/salmon_reads.log`
        fi
        ST=`cat $i/strand.txt | grep STRAND | awk '{print $8}' | uniq`
        ...
        NF=`grep "^ENS" $i/feature_counts.tsv | awk '{sum+=$7} END {printf "%d\n",sum}'`
        NR=`grep "^ENS" $i/rsem.genes.results | awk '{sum+=$5} END {printf "%d\n",sum}'`
        SA=`grep "^ENS" $i/salmon_aln/quant.genes.sf | awk '{sum+=$5} END {printf "%d\n",sum}'`
        SR=`grep "^ENS" $i/salmon_reads/quant.genes.sf | awk '{sum+=$5} END {printf "%d\n",sum}'`
        echo -e "$i\t$PE\t$ST\t$LT\t$RL\t$ML\t$N1\t$N2\t$N3\t$P1\t$P2\t$P3\t$P4\t$P5\t$P6\t$NS\t$NF\t$NR\t$SA\t$SR"
    fi
done 
